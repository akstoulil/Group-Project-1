...
int main()
{
	...
	while (gameContinue == true) {
		...
		cout << "Your bet is " << chipsToBet << " chips.";
		...
		while (playerTurn == true && user.getCard_total() <= maxCardValue) {
			...
			if (playerChoice == 1) {
				...
				user.addCard(DeckObject);
				playerTurn = false;
				cout << "Your bet total is " << chipsToBet << " chips.";
		...
		PlayerClass user2 = PlayerClass::PlayerClass();
		user2.dealerInitializeHand(DeckObject);

		while (aiTurn == true && user2.getCard_total() <= maxCardValue) {

			if (user2.getCard_total() < dealerMinCardValue) {

				user2.addDealerCard(DeckObject);
